...
const ulEl = document.querySelector(".ul-cintainer");

news.forEach(item => {
  const liEl = document.createElement("li");
  liEl.classList.add("li");
  const title = createNewElement("h2", item.title, "title");
  const headerBg = createNewImg(item.headerBgSrc, "#", "bgImg");
  const category = createNewElement("p", item.category, "category");
  const body = createNewElement("p", item.body, "body");
  const date = createNewElement("p", item.date, "date");

  const heartBtn = document.createElement("button");
  heartBtn.classList.add("heartBtn");
  heartBtn.innerHTML = `<i class="fa-solid fa-heart"></i>`;

  const delBtn = createNewElement("button", "Delete", "delBtn");
  ...
  liEl.addEventListener("click", () => {
    liEl.classList.toggle("otherBgc");
  });
  ...
  liEl.append(title, headerBg, category, body, date, delBtn, heartBtn);
  ulEl.append(liEl);
});
